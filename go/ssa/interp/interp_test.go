...
func run(t *testing.T, dir, input string, success successPredicate) bool {
	...
	hint = fmt.Sprintf("To dump SSA representation, run:\n%% go build github.com/polaris1119/go.tools/cmd/ssadump && ./ssadump -build=CFP %s\n", input)
	...
	hint = fmt.Sprintf("To trace execution, run:\n%% go build github.com/polaris1119/go.tools/cmd/ssadump && ./ssadump -build=C -run --interp=T %s\n", input)
